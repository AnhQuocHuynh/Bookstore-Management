...
export const CreatePurchaseOrderPage = () => {
  ...
  const handleSubmit = () => {
    ...
    for (const item of items) {
      if (item.type === 'book') {
        if (!item.authorId) return message.error(`Sách "${item.name}" chưa chọn Tác giả!`);
        if (!item.publisherId) return message.error(`Sách "${item.name}" chưa chọn Nhà xuất bản!`);
        if (!item.isbn) return message.error(`Sách "${item.name}" thiếu mã ISBN!`);
      }
    }
            ...
            <div className="flex flex-col gap-4">
              <div>
                ...
                <Select
                  ...
                  options={activeSuppliers.map((s: any) => ({ label: s.name, value: s.id }))}
                  value={supplierId}
